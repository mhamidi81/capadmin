 ...
 */
	public function admin_save_request_decision() {
		...
		$message =__d('request_managment', "Une erreur est survenue lors de l'aprobation de l'agréement! ");
		$result = 'error';
			...
			else
			{
				...
				if($judgment == 'Favorable')
				{
					if($this->Request->setStatus('granted', $id, $description)){
						$message = __d('request_managment', "L'agréement a été aprobé au bénéficiaire avec succès");
						$result = 'success';
					}
				}
 ...
 */
	public function admin_index() {
		...
		$this->set(compact('counselors', 'statuses', 'user_statuses'));
	}
